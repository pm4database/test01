...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem('📁 ตั้งค่า Google Drive Folders', 'setupDriveFoldersOnly')
...
function _runSetup(config) {
  ...
  // ── Guard: แจ้งเตือนถ้าเคย Setup แล้ว (แต่ไม่ block — idempotent) ──
  if (_isAlreadySetup(conn)) {
    _log('');
    _log('ℹ️ พบระบบที่เคย Setup แล้ว — จะทำ Re-Setup (idempotent)');
    _log('ℹ️ DB Schema จะใช้ IF NOT EXISTS / ON CONFLICT DO NOTHING');
  }
...
/**
 * 📁 setupDriveFoldersOnly() — สร้าง Google Drive Folders เท่านั้น
 * ใช้กรณีที่ initialSetup สร้าง DB ไปแล้ว แต่ Drive folders ยังไม่ถูกสร้าง
 * หรือต้องการสร้าง folders ใหม่ (idempotent — ถ้ามีอยู่แล้วจะใช้ของเดิม)
 */
function setupDriveFoldersOnly() {
  _log('📁 ═══════════════════════════════════════════════');
  _log('📁 สร้าง Google Drive Folders');
  _log('📁 ═══════════════════════════════════════════════');

  var config = _getConfig();
  if (!config) {
    _log('❌ ยังไม่ได้ตั้งค่า Config — กรุณารัน setupConfig() ก่อน');
    _showConfigDialog();
    return;
  }
  ...
  // สร้าง Drive Folders
  _log('');
  _log('📁 กำลังสร้าง/ตรวจสอบ Google Drive Folders...');
  var folders = _setupDriveFolders();

  // บันทึก Folder IDs กลับ Supabase
  _log('');
  _log('💾 บันทึก Folder IDs กลับ Supabase...');
  ...
  _log('🎉 ═══════════════════════════════════════════════');
  _log('   📁 Root:       ' + folders.rootId);
  _log('   📁 Templates:  ' + folders.templatesId);
  _log('   📁 Generated:  ' + folders.generatedId);
  _log('   📁 ZIP:        ' + folders.zipId);
  _log('   📁 Temp:       ' + folders.tempId);
  _log('   🔗 ' + folders.rootUrl);
}
...
function _saveDriveConfig(conn, folders) {
  ...
  // ── บันทึกลง Script Properties ด้วย (สำหรับ DriveAPI.gs อ่านเร็ว) ──
  var props = PropertiesService.getScriptProperties();
  props.setProperties({
    'DRIVE_ROOT_FOLDER': folders.rootId,
    'DRIVE_TEMPLATE_FOLDER': folders.templatesId,
    'DRIVE_GENERATED_FOLDER': folders.generatedId,
    'DRIVE_ZIP_FOLDER': folders.zipId,
    'DRIVE_TEMP_FOLDER': folders.tempId
  });

  _log('   ✅ บันทึก Drive Config — สำเร็จ (Supabase + Script Properties)');
}
